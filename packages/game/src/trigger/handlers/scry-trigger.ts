...
// ScryTrigger — Wave 16. Forge `T:Mode$ Scry`.
// Fires when a player scries (any amount).
//
// Forge pattern:
//   T:Mode$ Scry | ValidPlayer$ You | Execute$ TrigEffect
//     | TriggerDescription$ Whenever you scry, ...
//
// Engine event: "Scry" — already in core taxonomy.
import type {
  AbilityAst,
  GameEvent,
  PlayerSeat,
...
export class ScryTrigger extends TriggerHandler {
  static override readonly mode = "Scry";
  ...
  override build(ast: TriggerAst, ctx: TriggerBuildContext): TriggeredAbility {
    ...
    const ta: TriggeredAbilityWithResolver = {
      ...
      matches(event: GameEvent): boolean {
        if (event.kind !== "Scry") return false;
        const { playerSeat, count } = event.payload as {
      ...
      resolver: {
        *resolve(gameUnknown: unknown): Generator<unknown, void, unknown> {
          ...
          if (!sv) {
            throw new Error(
              `ScryTrigger: Execute$ SVar '${executeKey}' not found on ${sourceCard.paperCard.name ?? "?"}`,
            );
          }
          if (sv.kind !== "ability" || !sv.ability) {
            throw new Error(
              `ScryTrigger: Execute$ '${executeKey}' is not an ability SVar on ${sourceCard.paperCard.name ?? "?"}`,
            );
          }
...
triggerHandlerRegistry.register(ScryTrigger);
