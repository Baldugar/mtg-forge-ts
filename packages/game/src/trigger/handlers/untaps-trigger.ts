...
// UntapsTrigger — Wave 16. Forge `T:Mode$ Untaps`.
// Fires when a card untaps (Awakening Zone-style "whenever ~ untaps" effects).
//
// Forge pattern:
//   T:Mode$ Untaps | ValidCard$ Card.Self | Execute$ TrigEffect
//     | TriggerDescription$ Whenever this creature untaps, ...
//
// Engine event: "CardUntapped" — already in core taxonomy.
import type {
  AbilityAst,
  EntityId,
  GameEvent,
...
export class UntapsTrigger extends TriggerHandler {
  static override readonly mode = "Untaps";
  ...
  override build(ast: TriggerAst, ctx: TriggerBuildContext): TriggeredAbility {
    ...
    const ta: TriggeredAbilityWithResolver = {
      ...
      matches(event: GameEvent): boolean {
        if (event.kind !== "CardUntapped") return false;
      ...
      resolver: {
        *resolve(gameUnknown: unknown): Generator<unknown, void, unknown> {
          ...
          if (!sv) {
            throw new Error(
              `UntapsTrigger: Execute$ SVar '${executeKey}' not found on ${sourceCard.paperCard.name ?? "?"}`,
            );
          }
          if (sv.kind !== "ability" || !sv.ability) {
            throw new Error(
              `UntapsTrigger: Execute$ '${executeKey}' is not an ability SVar on ${sourceCard.paperCard.name ?? "?"}`,
            );
          }
...
triggerHandlerRegistry.register(UntapsTrigger);
