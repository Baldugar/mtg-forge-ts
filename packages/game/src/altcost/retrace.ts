...
//     "discard a land card" cost is documented under the closure note
//     below — the cast pipeline does not yet plug per-altcost additional
...
//
// Out-of-scope (Wave 118 closure note) — Wiring the additional
// Discard<1/Land.YouCtrl+InHand> cost into the cast pipeline mirrors
// jump-start's discard wiring: it lives on the cost-pipeline additional-
// costs surface (kicker/multikicker style), and porting Forge's
// CostAdjustment.adjust into stepDetermineTotalCost is the follow-up.
// The durable contract here — `altCostUsed = "Retrace"` opening the
// graveyard-cast lane — is the read side every consumer reads.
...
export const Retrace: AltCost = {
  ...
  modifyCastContext(ctx: CastContext, _sa: SpellAbility, _game: Game): void {
    ...
    // graveyard cast lane and (out-of-scope per closure note) attaches
    // an additional Discard<1/Land.YouCtrl> cost. Instants/sorceries
    // return to the graveyard normally on resolution; no
    // alternativeZoneDestination.
